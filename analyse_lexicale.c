...
/* Les variables et fonctions locales au module */
   ...
   void reconnaitre_lexeme() {
      typedef enum {E_INIT, E_id, E_ENTIER, E_FIN} Etat_Automate ;
    ...
    while (etat != E_FIN) {
            ...
            case LETTRE:
				...
				case SYMBOLE: 
                     ...
                     case ')':
                        ...
                        break;
                     case ';':
                        lexeme_en_cours.nature = pVirg;
                        break;
                     case ',':
                        lexeme_en_cours.nature = virg;
                        break;
                     case '!':
                        lexeme_en_cours.nature = dif;
                        break;
                     case '{':
                        lexeme_en_cours.nature = acoo;
                        break;
                     case '}':
                        lexeme_en_cours.nature = acof;
                        break;
   	       		  default:
        ...
        case E_id: // reconnaissance d'un identificateur (peut pas commencer par un chiffre)
            switch (nature_caractere(caractere_courant()))
            {
            case LETTRE:
            case CHIFFRE:
   ...
   Nature_Caractere nature_caractere (char c) {
      ...
      if (est_identificateur(c)) return IDF;
      if (est_affectation(c)) return AFF;
      if (est_separateur(c)) return SEPAFF;
	   return ERREUR_CAR ;
   }
   /* --------------------------------------------------------------------- */
   ...
   /* --------------------------------------------------------------------- */
      // vaut vrai ssi c designe un caractere correspondant a un séparateur attention pas de chaîne de caractère
   int est_identificateur(char c){
      return (c >='a' && c <='z'); 
      // || (c >='A' && c <='Z');
   ...
   char *Nature_vers_Chaine (Nature_Lexeme nature) {
	switch (nature) {
      ...
      case egal: return "Egalité";
      case dif : return "pas égale";
      case pVirg : return " point virgule";
      case virg : return "virgule";
      case inf : return "inférieur";
      case sup: return "supérieur";
      case PARO: return "parenthèse ouverte";
      case PARF: return "parenthèse Fermée";
